...
created: 2026-05-02
description: "花一小时让 AI 找到你的内容这几天有好几个小伙伴@我说，我的开源工具在他们问 AI 的时候被主动推荐了，啥也没做居然可以被收录，想着要不花一个小时把内容结构化整一整，应该会更好，于是整好以后，快速发了一个速记推，但是内容结构不清晰，想着大家很感兴趣，那要不就整一个结构清晰的文章..."
tags:
  - "clippings"
---
![图像](https://pbs.twimg.com/media/HHO7aYTbUAAU9Nz?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HHO18OEacAAyuOr?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HHO2GdjbQAAHSjM?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HHO2S2ZboAAJfcr?format=jpg&name=large)

做完之后可以提交到 [directory.llmstxt.cloud](https://directory.llmstxt.cloud/)、[llmstxt.site](https://llmstxt.site/)，还有 GitHub 上的 llms-txt-hub 仓库提 PR。

这里我还做了一个有意思的事：各站点的 llms.txt 互相引用，形成一个网状结构。我维护着 [tw93.fun](https://tw93.fun/)、[weekly.tw93.fun](https://weekly.tw93.fun/)、[yobi.tw93.fun](https://yobi.tw93.fun/) 几个站点，每个站点的 llms.txt 都引用其他站点，AI 不管从哪个入口进来都能顺着链接找到其他内容。

![图像](https://pbs.twimg.com/media/HHO2dmWaEAAESCe?format=jpg&name=large)

这些改动需要等爬虫重新抓取才会生效，通常要几天。配好之后隔一段时间去 ChatGPT 搜一下自己的项目名，引用来源和描述准确度应该会有变化。

![图像](https://pbs.twimg.com/media/HHO2kIha4AAWlhL?format=jpg&name=large)

![图像](https://pbs.twimg.com/media/HHO21jraQAAC15-?format=jpg&name=large)
...
操作很简单：去 [Google Search Console](https://search.google.com/search-console) 用 DNS 或 HTML 文件验证你的域名，验证通过后提交 sitemap URL（通常是 [yoursite.com/sitemap.xml](https://yoursite.com/sitemap.xml)）。在"网页索引"报告里可以看到哪些页面已收录、哪些有问题。如果某个重要页面没被收录，用"网址检查"工具手动请求编入索引。
...
IndexNow 的接入方式是在站点根目录放一个 API key 文件，然后在内容更新时向 [api.indexnow.org/indexnow](https://api.indexnow.org/indexnow) 发一个 POST 请求，把变更的 URL 列表发过去，几分钟内 Bing 就会来抓取。很多静态站点生成器和 CMS 有 IndexNow 插件可以直接用。

![图像](https://pbs.twimg.com/media/HHO3GpUbwAAmb5K?format=jpg&name=large)
...
Perplexity 在海外的用户量比大伙想的要大，他们有一个出版者计划，可以去 [pplx.ai/publisher-program](https://pplx.ai/publisher-program) 提交表单，通过之后有收入分成 80/20，还能看到引用分析数据。
...
![图像](https://pbs.twimg.com/media/HHO3ImTaUAAZMV9?format=jpg&name=large)

JSON API 返回的结构化数据，包含项目信息和实时 GitHub 统计：

![图像](https://pbs.twimg.com/media/HHO3RWYb0AAY2Aq?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HHO3cUpakAAZTfm?format=jpg&name=large)

URL 结构很重要，/projects/pake 在模型读一行字之前就告诉它这个页面是关于什么的，/page?id=47 什么都没说。

## 把结构化数据同步到主域名

子域名的权重不如根域名。AI 爬虫发现了 [example.com](https://example.com/) 不一定会自动去找 [docs.example.com](https://docs.example.com/) 或 [api.example.com](https://api.example.com/)。如果你的 llms.txt、项目页面、API 数据分散在多个子域名上，AI 可能只看到其中一部分。

解决方法是把关键的结构化数据镜像到主域名上，让 [example.com/llms.txt](https://example.com/llms.txt)、[example.com/projects/xxx.md](https://example.com/projects/xxx.md)、[example.com/api/projects.json](https://example.com/api/projects.json) 都在同一个域名下。AI 爬虫通过搜索索引发现你的主站，然后在同一个域名里就能拿到所有数据。实现方式可以是 CI 定时同步、构建时拉取、或者反向代理，选最适合你部署架构的就行。我用的是 GitHub Action 每天凌晨把子站数据同步到博客仓库。

![图像](https://pbs.twimg.com/media/HHO3lrAb0AAj6_p?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HHO37Vya4AAlSLh?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HHO32FWasAErIPs?format=jpg&name=large)
